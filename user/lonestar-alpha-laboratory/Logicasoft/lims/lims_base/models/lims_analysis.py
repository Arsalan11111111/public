...
class LimsAnalysis(models.Model):
    ...
    note = fields.Text('Note', help='Additional note')
    ...
    internal_comment = fields.Html('Internal comment', tracking=True,
                                   help="Additional comment addressed to the internal team "
                                        "(won't be printed on analysis report)")
    external_comment = fields.Html('External comment', tracking=True,
    ...
    def get_value_of_result_id(self, result_id):
        ...
        if ('limit_result_ids' in result_id._fields or 'limit_compute_result_ids' in result_id._fields) and result_vals.get('value'):
            limit_value, limit = self.get_limited_value(result_id, val_formula, digits)
            result_vals.update({
                'limit_value': limit_value,
                'limit_message': limit.message if limit else ''
            })
    ...
    def create(self, vals_list):
        ...
        request_ids = records.mapped('request_id')
        if request_ids:
    ...
    def create_sop(self):
        ...
        for record in self:
            ...
            for method_id in method_ids:
                sop_vals = record.sudo().get_sop_vals(method_id)
                if sop_vals:
    ...
    def do_cancel(self, cancel_reason='', cancel_stage_id=False):
        ...
        if cancel_stage_id and analysis:
            ...
            sop_ids = analysis.mapped('sop_ids').filtered(lambda s: s.stage_id.type != 'cancel')
            if sop_ids:
                sop_ids.do_cancel()
            request_ids = analysis.mapped('request_id')
            if request_ids :
                request_ids.compute_analysis_state()
                request_ids.check_done_state()
    ...
    def do_rework(self):
        ...
        sample_id = self.env['lims.analysis.request.sample'].sudo().search([('analysis_id', '=', self.id)])
        if sample_id:
