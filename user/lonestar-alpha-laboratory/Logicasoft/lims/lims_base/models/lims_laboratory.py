...
class LimsLaboratory(models.Model):
    ...
    def unlink(self):
        default_lab = self.env.ref('lims_base.default_laboratory')
        if default_lab and default_lab.id in self.ids:
            raise exceptions.ValidationError(_("You can't delete default laboratory. Archive it instead."))
