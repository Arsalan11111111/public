...
class LimsSop(models.Model):
    ...
    def do_wip(self):
        ...
        """
        records = self
        authorised_sop_stage = ['draft', 'plan', 'todo']
        if self.env.context.get('from_tree_view'):
            records = self.filtered(lambda s: s.stage_id.type in authorised_sop_stage)
        ...
        for record in records:
            ...
            if self.env.context.get('from_from_view'):
                record.filtered(lambda r: r.rel_type in authorised_sop_stage).stage_id = wip_stage_id
            else:
    ...
    def get_all_params(self):
        ...
        def stage_cond(result):
            return result.filtered(lambda r: r.rel_type not in ['cancel', 'rework'])
