...
class ProductProduct(models.Model):
    ...
    additional_pack_ids = fields.Many2many('lims.parameter.pack', string='Replacement Parameter Pack')

    def compute_info_analysis(self):
        for record in self:
            analysis_id = self.env['lims.analysis'].search([('product_id', '=', record.id)])
            record.nb_analysis = len(analysis_id)
            record.nb_result = len(analysis_id.mapped('result_num_ids'))
            record.nb_compute_result = len(analysis_id.mapped('result_compute_ids'))
            record.nb_sel_result = len(analysis_id.mapped('result_sel_ids'))
            record.nb_text_result = len(analysis_id.mapped('result_text_ids'))
    ...
    def get_all_analysis_result_view(self):
        ...
        :return:
        """
        analysis_ids = self.env['lims.analysis'].search([('product_id', '=', self.id)])
        return {
            'name': _('Result'),
