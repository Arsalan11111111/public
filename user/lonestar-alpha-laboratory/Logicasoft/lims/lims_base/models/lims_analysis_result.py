...
class LimsAnalysisResult(models.AbstractModel):
        ...
        if not self.env.context.get('bypass_checks'):
            ...
            for record in self:
                ...
                result_compute = record.analysis_id.result_compute_ids.filtered(
                    lambda c: c.rel_type not in ['cancel', 'rework'] and
    ...
    def reset_compute_result(self):
        ...
        for record in self:
            if record.analysis_id.result_compute_ids:
                result_compute = record.analysis_id.result_compute_ids.filtered(
                    lambda c: c.rel_type not in ['cancel', 'rework'])
    ...
    def get_limits_from_parameter(self):
    ...
    def get_limit_result_ids(self):
        ...
        :return:
        """
        return self.get_limits_from_parameter()
    ...
    @staticmethod
    def sigfig_decimal(input_value, n_decimal=2, sep_dec='.', sep_tho='', output_type=str, notation='standard'):
        """
    ...
    @staticmethod
    def sigfig_significant(input_value, n_significant=4, sep_dec='.', sep_tho='', output_type=str, notation='standard'):
        """
    ...
    @staticmethod
    def sigfig_uncertainty(input_value, u_value, sep_dec='.', sep_tho='', output_type=str, notation='standard'):
        """
    ...
    def sigfig_format(self, input_value, method='decimal', n_decimal=2, n_significant=4, sep_dec='.', sep_tho='',
                      input_u=0.0, output_type=str, notation='standard'):
        """
        ...
        :return:
        """
        if method in ['scientific', 'engineering']:
            notation = method
            method = 'both'
        if method == 'both':
        ...
        if method == 'decimal':
            input_value = self.sigfig_decimal(input_value, n_decimal, sep_dec, sep_tho, output_type, notation)
        elif method == 'significant_figure':
            input_value = self.sigfig_significant(input_value, n_significant, sep_dec, sep_tho, output_type, notation)
        elif method == 'uncertainty' and input_u != 0.0:
            input_value = self.sigfig_uncertainty(input_value, input_u, sep_dec, sep_tho, output_type, notation)
        return input_value

    def format_result(self, value, method_parameter_charac, lang=False, input_u=0.0, loq_mode=False,
                      notation='standard', options=False):
        ...
        :param notation: can be = ['scientific', 'engineering', 'standard'] by default standard
        :param options:
        :return:
        """
        if loq_mode:
        ...
        sep_dec = language.decimal_point or '.'
        sep_tho = language.thousands_sep or ''
    ...
    def open_wizard_history(self):
        ...
        return {
            ...
            'context': {
                'default_method_param_charac_id': self.method_param_charac_id.id,
    ...
    def prepare_result_log(self, log, result_reason_id=False):
        ...
        if result_reason_id:
            vals['result_reason_id'] = result_reason_id
        return vals
    ...
    def get_result_vals(self, specific_vals=False):
        """
