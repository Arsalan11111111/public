...
class ResConfig(models.TransientModel):
    ...
    is_parameter_and_pack_protected = fields.Boolean('Parameter characteristics and Pack validated is protected',
                                                     ...
                                                     help='Enable protection of validated characteristic parameters '
                                                          'and validated packs.')
...
def check_identity_lgk(fn):
    """
    'Extended' check_identity decorator to be configurable minimum time.
    ...
    def wrapped(self):
        ...
        configuration = request.env['ir.config_parameter'].sudo().get_param(param_ask_psw, default='False')
        if configuration != 'True':
            return fn(self)
        configuration_time = abs(
                int(request.env['ir.config_parameter'].sudo().get_param('time_between_check_identity'))) or 0.01
        # ! Can not really be 0.00, avoid eternal security reason loop. (Tolerance 1ms) between time and session
