...
class LimsAnalysisComputeResult(models.Model):
    ...
    def add_parameter_values(self, method_param_charac_id, vals):
        ...
        vals.update({
            ...
            'correspondence_ids': [Command.create(correspondence_id.get_correspondence_dictionary()
    ...
    def get_result_vals(self, specific_vals=False):
        res = super().get_result_vals(specific_vals=specific_vals)
        limits = self.get_limits_from_parameter()
        if 'limit_results' in specific_vals:
            limits = specific_vals['limit_results']
        res['limit_compute_result_ids'] = [Command.create(limit.get_values()) for limit in limits]
        if 'correspondences' in specific_vals:
            res['correspondence_ids'] = [Command.create(correspondence.get_correspondence_dictionary()) for
                                         correspondence in specific_vals['correspondences']]
        return res
    ...
    def check_computed_result(self, numeric_result=False, compute_result=False):
        ...
        for rec in result:
            ...
            computation_ids = rec.analysis_id.result_compute_ids.filtered(
                lambda r: r.rel_type not in ['cancel', 'rework'] and r.correspondence_ids.filtered(
