...
class AddParametersWizard(models.TransientModel):
    ...
    def get_line_ids(self, default_analysis_id=None):
        ...
        for result_id in analysis_id.result_num_ids.filtered(lambda r: r.rel_type not in ['cancel', 'rework']):
            vals = {
        ...
        for result_id in analysis_id.result_sel_ids.filtered(lambda r: r.rel_type not in ['cancel', 'rework']):
            vals = {
                'method_param_charac_id': result_id.method_param_charac_id.id,
                'pack_id': result_id.pack_id.id,
                'in_analysis': True,
        ...
        for compute_result_id in analysis_id.result_compute_ids.filtered(lambda r: r.rel_type not in ['cancel',
                                                                                                      'rework']):
            vals = {
            ...
            for method_param_charac_id in compute_result_id.method_param_charac_id.conditional_parameters_ids:
                if method_param_charac_id not in line_ids.mapped('method_param_charac_id'):
                    ...
                    line_ids += line_obj.new(vals)
        for result_id in analysis_id.result_text_ids.filtered(lambda r: r.rel_type not in ['cancel', 'rework']):
            vals = {
                'method_param_charac_id': result_id.method_param_charac_id.id,
                'pack_id': result_id.pack_id.id,
                'in_analysis': True,
