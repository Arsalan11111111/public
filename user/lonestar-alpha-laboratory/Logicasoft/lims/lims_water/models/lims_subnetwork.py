...
class LimsSubnetwork(models.Model):
    _name = 'lims.subnetwork'
    _description = 'Lims Subnetwork'
    ...
    sampling_point_ids = fields.One2many('lims.sampling.point', 'subnetwork_id', 'Sampling Point', readonly=True)
    ...
    network_id = fields.Many2one('lims.network', 'Network', required=True)

    def count_analysis(self):
        for record in self:
            record.total_analysis = sum(sampling_point_id.total_analysis
                                        for sampling_point_id in record.sampling_point_ids)

    def count_compute_result(self):
        for record in self:
            record.total_compute_result = sum(sampling_point_id.total_compute_result
                                              for sampling_point_id in record.sampling_point_ids)

    def count_sel_result(self):
        for record in self:
            record.total_sel_result = sum(sampling_point_id.total_sel_result
                                          for sampling_point_id in record.sampling_point_ids)

    def count_text_result(self):
        for record in self:
            record.total_text_result = sum(sampling_point_id.total_text_result
                                           for sampling_point_id in record.sampling_point_ids)

    def count_result(self):
        for record in self:
            record.total_result = sum(sampling_point_id.total_result
                                      for sampling_point_id in record.sampling_point_ids)
