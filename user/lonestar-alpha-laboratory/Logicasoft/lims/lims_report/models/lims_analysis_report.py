...
class LimsAnalysisReport(models.Model):
    ...
    external_comment = fields.Html('External comment',
                                   help='Additional comment addressed to the customer '
                                        '(will be printed on analysis report)')
    ...
    internal_comment = fields.Html('Internal comment', help="Additional comment addressed to the internal team "
    ...
    date_validated = fields.Datetime(tracking=True, help="The date on which the report is validated")
    ...
    option_print_value = fields.Boolean('Print Value', default=True, tracking=True,
                                        help="If checked, the value of the parameter of the analysis result will be "
                                             "printed.")
    option_print_uom = fields.Boolean('Print UoM', default=True, tracking=True,
                                      help="If checked, the 'UdM' of the parameter of the analysis result will be "
                                           "printed.")
    ...
    option_print_attribute = fields.Boolean(string="Print attribute", tracking=True,
                                            help="If checked, all test's attributes marked as 'To print', "
                                                 "will be on report.")
    option_print_sample_comment = fields.Boolean(string="Print sample comment", tracking=True,
                                                   help="If checked, the external comment of the report's samples "
                                                        "will be printed")

    @api.onchange('previous_analysis_report_id')
    def _onchange_previous_analysis_report_id(self):
        for record in self.filtered("previous_analysis_report_id"):
            record.external_comment = record.laboratory_id.note_report_replaced
    ...
    def get_result_vals(self, analysis, parameter_print):
        ...
        for result in results.filtered(lambda r: r.method_param_charac_id
            ...
            result_vals['sop_id'] = result.sop_id.external_comment if result.sop_id else False
    ...
    def do_validated(self):
        ...
        if len(set(self.mapped('report_analysis_line_ids.analysis_id.stage_id.type') + ['validated2', 'cancel'])) == 2:
            ...
            self.date_validated = fields.Datetime.now()
        else:
        ...
        if self.env['ir.config_parameter'].sudo().get_param('is_automatic_customer_follower'):
            ...
            if updated_values.get('partner_contact_ids'):
                ...
                if isinstance(contact_vals[0], list):
                    ...
                    if len(contact_vals[0]) == 3 and contact_vals[0][2] > 0:
    ...
    def get_internal_comment(self):
        self.ensure_one()
        return self.internal_comment if bool(html2plaintext(self.internal_comment)) else False

    def get_external_comment(self):
        self.ensure_one()
        return self.external_comment if bool(html2plaintext(self.external_comment)) else False
