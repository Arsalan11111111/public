...
class LimsAnalysisRequest(models.Model):
    ...
    def onchange_invoice_to(self):
        for record in self:
            invoice_partners = record.partner_id and record.partner_id.child_ids.filtered(lambda c: c.type == 'invoice')
            record.invoice_to_id = invoice_partners and invoice_partners[0] or record.partner_id
