...
class MRPProduction(models.Model):
    ...
    def get_analysis(self):
        analysis = self.env['lims.analysis'].search([('customer_ref', 'in', self.mapped('name'))])
        analysis += (self.move_raw_ids.move_line_ids.analysis_ids + self.finished_move_line_ids.analysis_ids).filtered(
            lambda a: a not in analysis
        )
        return analysis
    ...
    @api.depends('product_id', 'product_id.lims_for_analysis', 'product_id.matrix_id', 'rel_create_analysis')
    ...
    def create_analysis(self, lot_id=False, parameters_ids=False):
        ...
        new_analysis = self.env['lims.analysis'].create({
            ...
            'category_id': self.picking_type_id.analysis_category_id.id
        })
