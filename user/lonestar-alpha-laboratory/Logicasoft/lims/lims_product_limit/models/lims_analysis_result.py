...
class LimsAnalysisResult(models.AbstractModel):
    ...
    def get_limits_from_product(self, product):
        product = product or (self.analysis_id and self.analysis_id.product_id)
    ...
    def get_limit_result_ids(self):
        ...
        :return:
        """
        return self.get_limits_from_product(self.analysis_id.product_id) or super().get_limit_result_ids()
    ...
    def get_report_limit_value(self, method_param_charac_id):
        ...
        report_limit_value = self.get_report_limit_value_from_product(method_param_charac_id, analysis.product_id)
        if report_limit_value:
            return report_limit_value
        return super().get_report_limit_value(method_param_charac_id)

    def get_report_limit_value_from_product(self, method_param_charac_id, product):
        """
        ...
        if param_char_product_ids:
            param_char_product_id = param_char_product_ids[-1]
            return param_char_product_id._fields.get('report_limit_value')._get_stored_translations(
                param_char_product_id)
