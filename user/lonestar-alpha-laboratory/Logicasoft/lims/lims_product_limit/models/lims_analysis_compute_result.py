...
class LimsAnalysisComputeResult(models.Model):
    ...
    def create_limit_compute_result(self):
        """
        Create the limits for the result
        :return:
        """
        for record in self:
            limit_ids = record.get_limit_result_ids()
            record.create_limit(limit_ids)
            if limit_ids and not limit_ids.filtered(lambda l: l.type_alert == 'limit') and \
    ...
    def get_result_vals(self, specific_vals=False):
        ...
        limits = self.get_limits_from_product(self.analysis_id.product_id)
        if limits:
