...
class LimsAnalysisReport(models.Model):
    ...
    @api.depends('report_analysis_line_ids.analysis_id')
    def get_report_images(self):
        attachments = self.env['ir.attachment'].search([
            ('res_id', 'in', self.report_analysis_line_ids.analysis_id.ids),
            ('mimetype', 'like', 'image'),
            ('res_model', '=', 'lims.analysis'),
        ])
        for record in self:
            record.report_image_attachment_ids = attachments.filtered(
                lambda a: a.res_id in record.report_analysis_line_ids.analysis_id.ids
            )
    ...
    def get_all_images(self, analysis):
        self.ensure_one()
        images = []
        if self.image_01:
            images.append({'binary': self.image_01, 'text': self.text_image_01})
        if self.image_02:
            images.append({'binary': self.image_02, 'text': self.text_image_02})
        if self.image_03:
            images.append({'binary': self.image_03, 'text': self.text_image_03})
        for attachment in self.get_attachment_for_analysis(analysis):
            images.append({'binary': attachment.datas, 'text': attachment.description})
        return images
