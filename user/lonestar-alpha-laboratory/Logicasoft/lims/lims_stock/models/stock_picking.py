...
class StockPicking(models.Model):
    ...
    def get_state_analysis(self):
        for record in self:
            analysis = record.get_picking_analysis()
            states = analysis.mapped('state')

            analysis_states = False
            if 1 == len(set(states)):
                if 'conform' in states:
                    analysis_states = 'all_conform'
                elif 'not_conform' in states:
                    analysis_states = 'all_not_conform'
            elif ('conform' in states) and ('not_conform' in states):
                analysis_states = 'both'
            record.analysis_states = analysis_states

    def get_picking_analysis(self):
        return self.move_ids_without_package.move_line_ids.analysis_ids
