...
class DuplicateTourWizard(models.TransientModel):
    ...
    def do_duplicate_tour(self):
        ...
        for val in vals_num:
            ...
            result.update({
                'limit_result_ids': [Command.create(limit.get_values()) for limit in
                                     tab_result[val.get('method_param_charac_id')].limit_result_ids]
            })
        for val in vals_se:
        ...
        for val in vals_ca:
            ...
            result.update({
                'limit_compute_result_ids': [Command.create(limit.get_values()) for limit in
