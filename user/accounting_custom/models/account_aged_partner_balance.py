...
class AgedPartnerBalanceCustomHandler(models.AbstractModel):
    ...
    _inherit = ['account.aged.partner.balance.report.handler', 'odoo.logger']
    ...
    def _aged_partner_report_custom_engine_common(self, options, internal_type, current_groupby, next_groupby, offset=0, limit=None):
        if self.env.company.aged_partner_ext_days_range:
            report = self.env['account.report'].browse(options['report_id'])
            report._check_groupby_fields((next_groupby.split(',') if next_groupby else [
            ...
            periods = [
                ...
                (minus_days(date_to, 121), minus_days(date_to, 180)),
                (minus_days(date_to, 181), minus_days(date_to, 365)),
                (minus_days(date_to, 366), minus_days(date_to, 730)),
                (minus_days(date_to, 731), False),
            ]
            ...
            tail_query, tail_params = report._get_engine_query_tail(
                offset, limit)
            query = f"""
            ...
            if not current_groupby:
                ...
                for query_res in query_res_lines:
                    ...
                    all_res_per_grouping_key.setdefault(
                        grouping_key, []).append(query_res)

                for grouping_key, query_res_lines in all_res_per_grouping_key.items():
                    rslt.append(
                        (grouping_key, build_result_dict(report, query_res_lines)))
