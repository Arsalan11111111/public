...
class MultipleProduct(models.TransientModel):
    ...
    def action_add_line(self):
        ...
        if active_model == 'account.move':
            ...
            current_id = self.env['account.move'].browse(
                active_id
            )

        for rec in self.product_list_ids:
            if active_model == 'account.move':
                if rec not in current_id.invoice_line_ids.product_id:
                    invoice_line = self.env[line].create({
                        'move_id': active_id,
                        'product_id': rec.id,
                        'price_unit': rec.list_price,
                        'quantity': 1,
                    })
                    # This method is called to get analytic accounts in invoice line
                    # This method is of module "atw_account_features"
                    invoice_line.get_analytic_accounts()

                else:
                    current_id.invoice_line_ids.filtered(
                        lambda self: self.product_id == rec).quantity += 1
            else:
