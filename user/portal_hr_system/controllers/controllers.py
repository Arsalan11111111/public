...
class CustomerPortalInherit(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        if omni_attendance_rec:
            data.update({
                ...
                'allowed_attendance':omni_employee_id.is_attendance_allowed
            })
        else:
            data.update({
                'omni_employee_id': omni_employee_id.name,
    ...
    def portal_my_attendance(self):
        ...
        return request.render("portal_hr_system.attendance_home", values)
    ...
    def list_method(self, page=1, sortby=None, filterby=None, search=None, search_in='all', groupby='none', **kw):
        ...
        return request.render("portal_hr_system.attendance_list_view", data)
...
class PortalLeaveKnk(CustomerPortal):
    ...
    def portal_payslip(self, page=1, sortby=None, filterby=None, search=None, search_in='all', groupby=None, **kw):
        ...
        return request.render("portal_hr_system.portal_my_leave_list", values)
    ...
    def apply_leave(self, **post):
        ...
        return request.render("portal_hr_system.portal_apply_leave", values)

    @http.route(['/save/leave'], type='http', auth="user", website=True)
    def save_leave(self, **post):
        field_list = ['start_date', 'end_date', 'reason', 'leave_type','image_upload']
        value = []
        domain = ['|', ('requires_allocation', '=', 'no'), '&', ('has_valid_allocation', '=', True), '&', ('virtual_remaining_leaves', '>', 0), ('max_leaves', '>', '0')]
        leave_type = request.env['hr.leave.type'].search(domain)
        ...
        if any([field not in post.keys() for field in field_list]) or not all(value) or not post:
            ...
            return request.render("portal_hr_system.portal_apply_leave", post)
        ...
        leavess = request.env['hr.leave'].create(vals)
        if 'image_upload' in post and post['image_upload']:
            image_file = post['image_upload']
            file_content = image_file.read()

            attachment = request.env['ir.attachment'].sudo().create({
                'name': image_file.filename,
                'datas':base64.b64encode(file_content),
                # 'datas_fname': image_file.filename,
                'res_model': 'hr.leave',
                'res_id': leavess.id,
            })
...
class CustomerPortal(CustomerPortal):
    ...
    def portal_my_loan(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
        values = self._prepare_portal_layout_values()
        partner = request.env.user.partner_id
        payslip_pool = request.env['hr.payslip'].sudo().search([])
        ...
        searchbar_sortings = {
            'name': {'label': _('Number'), 'order': 'number desc'},
        ...
        #        archive_groups = self._get_archive_groups('hr.payslip', domain)
        ...
        return request.render("portal_hr_system.portal_my_payslip", values)
    ...
    def portal_payslip_page(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        if payslip_sudo and request.session.get(
            ...
            body = _('Payslip viewed by customer')
            # _message_post_helper(res_model='dev.loan.loan', res_id=payslip_sudo.id, message=body,
            #                      token=payslip_sudo.access_token, message_type='notification', subtype="mail.mt_note",
            #                      partner_ids=payslip_sudo.employee_id.user_id.sudo().partner_id.ids)
        values = {
        ...
        return request.render('portal_hr_system.payslip_portal_template', values)
