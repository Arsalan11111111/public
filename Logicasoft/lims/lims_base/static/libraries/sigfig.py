...
def _arguments_parse(args, kwargs):
    ...
    keys = {'separator', 'separation', 'sep', 'format', 'sigfigs', 'decimals', 'uncertainty', 'cutoff', 'spacing',
            'spacer', 'decimal', 'output_type', 'output', 'type', 'style', 'prefix', 'notation', 'form', 'crop'}
    for key in kwargs:
        val = kwargs[key]
        if key not in keys:
        ...
        if key in {'sigfigs', 'decimals', 'cutoff', 'crop', 'spacing'}:
            ...
            if type(val) == bool or val in {'major', 'sci', 'eng'}:
                ...
                key, val, str([True, False, 'major', 'minor', 'all'])))
                continue
            given['output_type'] = str
            ...
            if val in notations:
                ...
                warn(
                    "expected format of %s, ignoring format of %s" % ([f for f in formats] + [o for o in outputs], val))
                given['output_type'] = type(args[0])
        else:
            given[key] = val
...
def round(*args, **kwargs):
    ...
    if 'uncertainty' in given and given[
        'separator'] == 'brackets' and unc.min_power() > 0 and 'external_brackets' not in given:
