...
class LimsAnalysisRequest(models.Model):
    ...
    def create_analysis(self, reception_date, sample_and_due_date, sample_ids=None):
        ...
        all_method_param = self.env['lims.method.parameter.characteristic']
        # Take all result from analysis (usefull to not re-create the result a second time)
        all_method_param += analysis_ids.result_num_ids.method_param_charac_id + analysis_ids.result_sel_ids.method_param_charac_id + \
                            analysis_ids.result_compute_ids.method_param_charac_id + analysis_ids.result_text_ids.method_param_charac_id
        method_param_from_res = analysis_ids.method_param_charac_ids.filtered(lambda x: x not in all_method_param) + \
                                analysis_ids.pack_ids.parameter_ids.method_param_charac_id.filtered(
                                    lambda x: x not in all_method_param)
        # Fill the array with result
        tab_result = self.fill_tab_result(self.sample_ids.filtered(lambda x: not x.analysis_id),
                                          method_param_from_res)
        ...
        for sample_id in sample_ids:
            ...
            analysis_parameters = analysis_id.result_num_ids.method_param_charac_id + \
                                  analysis_id.result_sel_ids.method_param_charac_id + \
                                  analysis_id.result_compute_ids.method_param_charac_id + \
                                  analysis_id.result_text_ids.method_param_charac_id
            all_pack = sample_id.pack_of_pack_ids.pack_of_pack_ids.pack_id
            all_pack += sample_id.pack_ids.filtered(lambda x: x.id not in all_pack.ids)
            for pack_id in all_pack:
                for method_param_id in pack_id.parameter_ids.method_param_charac_id. \
                        filtered(lambda x: x not in analysis_parameters):
                    ...
                    for conditional in method_param_id.conditional_parameters_ids:
                        self.create_result(analysis_id, conditional, tab_result, plan_stage_id, vals_num, vals_se,
                                           vals_ca, vals_tx)
            for method_param_id in sample_id.method_param_charac_ids.filtered(lambda x: x not in analysis_parameters):
                self.create_result(analysis_id, method_param_id, tab_result, plan_stage_id, vals_num, vals_se, vals_ca,
                                   vals_tx)
                for conditional in method_param_id.conditional_parameters_ids:
                    self.create_result(analysis_id, conditional, tab_result, plan_stage_id, vals_num, vals_se,
                                       vals_ca, vals_tx)
            if sample_id.method_param_charac_ids not in sample_id.analysis_id.method_param_charac_ids:
                method_ids = sample_id.method_param_charac_ids.filtered(
                    lambda x: x not in sample_id.analysis_id.method_param_charac_ids)
                sample_id.analysis_id.write({'method_param_charac_ids': [(4, x.id) for x in method_ids]})
            if sample_id.pack_ids not in sample_id.analysis_id.pack_ids:
                pack_ids = sample_id.pack_ids.filtered(lambda x: x not in sample_id.analysis_id.pack_ids)
                sample_id.analysis_id.write({'pack_ids': [(4, x.id) for x in pack_ids]})
        # Create result
        for val in vals_num:
            result = result_nu_obj.with_context(lang=None, no_limit=True, bypass=True).create(val)
            result.update(result.get_result_vals(tab_result[val.get('method_param_charac_id')].limit_result_ids))
        for val in vals_se:
        ...
        for val in vals_ca:
            ...
            result.update(result.get_result_vals(
                tab_result[val.get('method_param_charac_id')].correspondence_ids,
                tab_result[val.get('method_param_charac_id')].limit_compute_result_ids))
        for val in vals_tx:
    ...
    def fill_tab_result(self, sample_ids, method_param=False):
        ...
        all_pack += sample_ids.pack_ids.filtered(lambda p: p.id not in all_pack.ids)
