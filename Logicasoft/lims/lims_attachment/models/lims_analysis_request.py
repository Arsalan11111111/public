...
class LimsAnalysisRequest(models.Model):
    ...
    def open_analysis_pictures(self):
        ...
        ctx.update({
            ...
            'default_res_model': 'lims.analysis.request',
        })
        pictures = self.report_image_attachment_ids.ids
        return {
            'name': 'Picture',
    ...
    def get_report_images(self):
        ...
        for record in self:
            report_image_attachment_ids = attachment_obj.search([('res_id', '=', record.id),
                                                                 ('mimetype', 'like', 'image'),
                                                                 ('res_model', '=', 'lims.analysis.request'),
                                                                 ])
            record.report_image_attachment_ids = report_image_attachment_ids
            record.pictures_count = len(report_image_attachment_ids)
