...
class LimsAnalysisNumericResult(models.Model):
    ...
    def check_result_conformity(self):
        ...
        for record in self.filtered(lambda r: r.limit_result_ids.filtered(lambda l: l.type_alert == 'plausibility')):
            plausible = True
            value = record.corrected_value
            if value or record.is_null:
                for limit in record.limit_result_ids.filtered(lambda l: l.type_alert == 'plausibility'):
