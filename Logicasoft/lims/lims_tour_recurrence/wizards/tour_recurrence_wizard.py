...
class TourRecurrenceWizard(models.TransientModel):
    _name = 'tour.recurrence.wizard'
    _description = 'Tour recurrence wizard'

    def get_day_in_month(self):
        return [(str(x), str(x)) for x in range(1, 32)]

    tour_id = fields.Many2one('lims.tour', string='Original Tour', readonly=True)
    interval = fields.Integer('Interval', required=True, default=1)
    rrule_type = fields.Selection(
        [('daily', 'Day(s)'), ('weekly', 'Week(s)'), ('monthly', 'Month(s)'), ('yearly', 'Year(s)')],
        'Period', required=True)
    end_type = fields.Selection([('count', 'Number of repetitions'), ('end_date', 'End date')], 'Until',
    ...
    byday = fields.Selection([('1', 'First'), ('2', 'Second'), ('3', 'Third'), ('4', 'Fourth'), ('5', 'Fifth'),
                              ('-1', 'Last')], 'By day')
    ...
    def create_tour(self, vals):
        self.tour_id.with_context(copy_all=True).copy(default=vals)
    ...
    def check_date_selected(self):
        return any(safe_eval('self.' + d, {'self': self}) for d in days)
            ...
            if self.rrule_type in ['daily', 'yearly'] or (self.rrule_type == 'monthly' and not self.month_by):
                ...
                while date_ok:
                    ...
                    if (self.end_type != ' count' and date_end and deadline <= date_end) or self.end_type == 'count':
                        ...
                        date_ok = False
        create_ids = self.env['lims.tour'].search([], order="id desc", limit=nb_create)
        return {
            'name': _('Tour created'),
