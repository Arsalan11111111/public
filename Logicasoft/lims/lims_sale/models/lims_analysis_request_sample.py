...
class LimsAnalysisRequestSample(models.Model):
    ...
    def update_parameter_vals(self, vals, section_name, product, parameter_ids=None, add_parameters=False):
    ...
    def _get_invoiceable_parameters(self):
        ...
        for record in self:
            ...
            method_param_charac_ids = record.method_param_charac_ids.filtered(
                lambda p: p not in record.pack_ids.parameter_ids.method_param_charac_id and p.id not in parameters_in_so)
            for parameter_id in method_param_charac_ids:
                product_id = parameter_id.get_product(sample_id=record)
                if parameter_id.billable and product_id:
                    record.update_parameter_vals(vals, parameter_id.tech_name, product_id, parameter_id, add_parameters=True)
        return vals
