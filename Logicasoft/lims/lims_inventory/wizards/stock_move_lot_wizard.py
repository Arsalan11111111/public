...
class StockMoveLotWizard(models.TransientModel):
    ...
    def default_get(self, fields_list):
        res = super(StockMoveLotWizard, self).default_get(fields_list)
        stock_move_id = self.env['stock.move'].browse(self.env.context.get('default_stock_move_id'))
        if stock_move_id:
            product_id = stock_move_id.product_id
            laboratory = False
            if product_id.pack_ids:
                laboratory = product_id.pack_ids[0].labo_id
            else:
                laboratory = stock_move_id.picking_id.picking_type_id.laboratory_company_id
            res.update({
                'stock_move_id': stock_move_id.id,
                'line_ids': [(0, 0, {'move_line_id': line, 'lot_id': line.lot_id.id, 'qty_done': line.qty_done})
                             for line in stock_move_id.move_line_nosuggest_ids],
    ...
    def confirm(self):
        if self.line_ids:
            return self.stock_move_id.create_analysis(laboratory=self.laboratory_id[0], stock_moves=self.line_ids)
