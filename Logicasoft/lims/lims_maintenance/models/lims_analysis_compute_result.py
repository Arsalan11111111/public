            ...
            if equipment_id:
                for record in self:
                    ...
                    if equipment_line_ids:
                        ...
                        self.update_result_vals(equipment_line, res_vals)
                    super().write(res_vals)
            # option 2 : equipment is removed
            else:
                for record in self:
                    res_vals = values.copy()
                    self.update_result_vals(record.method_param_charac_id, res_vals)
                    super().write(res_vals)
        else:
            super().write(values)
        return True

    def update_result_vals(self, update_object, result_vals):
        """
        Update result vals with either equipment line or parameter values
        """
        if update_object.loq:
            result_vals.update({'loq': update_object.loq})
        if update_object.lod:
            result_vals.update({'lod': update_object.lod})
        if update_object.u:
            result_vals.update({'u': update_object.u})
